...
export default function Login() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message.includes("wrong-password") ? "Noto‘g‘ri parol" : "Kirishda xatolik");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full p-8 border rounded-xl shadow-lg bg-white">
        <h2 className="text-3xl font-bold text-center mb-8">Tizimga kirish</h2>
        ...
        <p className="text-center mt-6 text-sm">
          Hisobingiz yo‘qmi? <Link href="/register" className="text-blue-600 font-medium">Ro‘yxatdan o‘tish</Link>
        </p>
      </div>
    </div>
  );
